...
**คำแนะนำ (ออปชัน — ไม่ใช่ข้อบังคับ):**
- **ถ้า agent ตัวไหนใช้บ่อย/หนัก → จะแยก API key ให้มันต่างหากก็ได้** เพื่อกระจายโควต้า ลดโอกาสชน rate limit
- ปกติ **แชร์ key เดียวหลาย agent/หลาย model ได้สบาย** — เป็น default ที่ยืดหยุ่น ใช้แบบนี้ได้เลย
- ต่าง provider (Claude/GPT) ต้องใช้ key ของเจ้านั้นอยู่แล้ว
- งาน production ที่ยิงหนัก → ใช้ paid tier (limit สูงกว่ามาก) หรือแยก project/key

**หมายเหตุ:** ปัจจุบันระบบเก็บ key **1 อันต่อ provider** (`.env`) แชร์ทุก agent — ซึ่ง**เพียงพอและยืดหยุ่นดีอยู่แล้ว**.
การแยก key ต่อ agent (per-agent key store) เป็นแค่ทางเลือกถ้าวันหน้าอยากกระจายโควต้าจริง ๆ — **ไม่ได้วางแผนบังคับ** ไม่ทำก็ได้
